...
log_file_exporter = 'show_run_exporter_'+time+'.yaml'
log_file_exporter_lines = 'show_run_exporter'+time+'.yaml'
missing_config= 'data_missing_'+time+'.csv'
failed='failed_host_'+time+'.csv'

#script Section
print('Start time: ', start_time)
def find_exporter_name(host_ip):
    ...
    if output:
        if 'invalid' in output:
            ...
            listfied_output = output.split('exporter')
            exporter_name = listfied_output[1]

            with open(log_file_exporter, "a") as logs:
        ...
        with open(inventory, "r") as inv:
            ...
            for platform in host_file['platform']:
                if platform['name']=='cisco_ios':
                    ...
                    command= 'show  running-config  | i flow exporter'
                    ...
                    for x in range(0, host_len):
                        ...
                        try:
                            ...
                            th = threading.Thread(target=find_exporter_name(host_ip), args=(host_ip))
                            threads.append(th)
                            th.start()
                            while True:
                                alive_cnt = 0
